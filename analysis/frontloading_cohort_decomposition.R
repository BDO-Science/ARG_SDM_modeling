...
# ---------------------------------------------------------------------------
# The redd draw is seeded - set.seed(123) at precompute.R:31 covers section 17,
# and nothing consumes the RNG stream in parallel before it - so it reproduces on
# a full re-run. But it was never saved, and app_data/sim_redds.rds is a stale
# artifact from an older pipeline version. The redd set behind the published
# egg_summary.rds is therefore not recoverable from the saved artifacts without
# re-running the whole pipeline. Part 0 quantifies the gap. (precompute.R now
# saves sim_redds and sim_future, so this applies to the published run only.)
#
