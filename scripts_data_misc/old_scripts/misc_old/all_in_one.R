...
# 2) helper: compute surv for one redd
compute_surv <- function(rdr, site, date_idx_env, temps_env, model, calib) {
  ...
  td    <- ceiling(egg_model_c(T0) + hatch_model_c(T0))
  slice <- temps_env[[site]][ pos + seq_len(td) - 1L ]
  if (model == "exp") tdm_exp_c(slice, calib)
  else                 tdm_lin_c(slice)
}
...
generate_SAR_vec <- function(n_years, opts) {
  ...
  # block timing: as before
  if (opts$timing == "block") {
    vec[-opts$block_years] <- opts$mean
  }
  
  # pulse timing: reset & clamp
  if (opts$timing == "pulse") {
    # step 1: reset everything to the mean
    vec[] <- opts$mean
    
    # step 2: turn the pulse list into valid integer indices
    yrs_idx <- as.integer(opts$pulse_years)
    yrs_idx <- yrs_idx[yrs_idx >= 1 & yrs_idx <= n_years]
    
    # step 3: only if there are any valid indices
    if (length(yrs_idx) > 0) {
...
env_ext_list     <- saveRDS(env_ext_list, "env_ext_list.rds")
df_all           <- saveRDS(df_all, "df_all.rds")
egg_summary      <- saveRDS(egg_summary, "egg_summary.rds")
surv_lookup_full <- saveRDS(surv_lookup_full, "surv_lookup_full.rds")
base_P_list      <- saveRDS(base_P_list, "base_P_list.rds")
S_seed           <- saveRDS(S_seed, "S_seed.rds")
stoch_SAR_opts   <- saveRDS(stoch_SAR_opts, "stoch_SAR_opts.rds")
generate_SAR_vec <- saveRDS(generate_SAR_vec, "generate_SAR_vec.rds")
simulate_variant <- saveRDS(simulate_variant, "simulate_variant.rds")
base_P           <- saveRDS(base_P, "base_P.rds")
