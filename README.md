...
# Deploy to shinyapps.io -- see SalmonCountR/DEPLOY.md
source("SalmonCountR/deploy.R")
arg_deploy(account = "reclamation-bdo-science")
```

The same code runs locally and on the server; nothing needs rearranging before
a deploy. `arg_deploy()` builds the app from the upload set first and refuses
...
```r
install.packages(c(
  # app  (app.R + global.R + years.R + functions.R) -- this is all the server needs
  "shiny", "shinyjs", "shinyWidgets", "DT", "tidyverse", "scales", "ggrepel",
  # precompute.R additionally (local only -- precompute.R is not deployed)
  "furrr", "future", "data.table", "MASS", "ordinal", "ggridges", "readxl", "here",
...
├── years.R             # Analysis-year registry + app-directory path resolution
...
├── deploy.R            # Verified one-command deploy (not itself deployed)
├── DEPLOY.md           # Deployment guide and setup
├── .rscignore          # Generated by deploy.R; keeps precompute files out of the bundle
